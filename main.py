...
# Cold power-up: the sound board boots slower than the Pico, and any
# command sent before it wakes is lost. The strap holds its BUSY line
# HIGH until the board boots and drives it LOW - so wait for that drop
# (a warm start sails straight through; give up after 8 s if no board).
while (busy.value() == config.BUSY_ACTIVE and
       time.ticks_diff(time.ticks_ms(), now) < 8000):
    time.sleep_ms(50)
