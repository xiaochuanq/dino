...
def test_visitor_wrapper_reads_its_own_laser():
    ...
    laser = FakeLaser()
    v = Visitor(laser, here_mm=1000, leave_mm=1500, passing_mm=3000,
                cooldown_ms=30 * S)
    # defaults samples=5, hold_ms=300: feed enough ticks to settle
    for t in range(0, 1000, 50):
        v.update(t)
    assert v.where == HERE
    laser.value = 3500                   # they walked away
    for t in range(1000, 4000, 50):
        v.update(t)
    assert v.where == AWAY               # the wrapper really reads its laser
