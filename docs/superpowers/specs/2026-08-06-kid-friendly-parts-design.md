# Dino kid-friendly rewrite — visitor/door/voice/eyes parts + 3-step loop
...
should see objects named after **concepts** — `visitor`, `door`, `voice`,
...
1. **SENSE** — each part reads its hardware and updates its state, translating
   ...
   here"; a blocked beam becomes "the door is open").
...
- **Donation** (person is here AND door opens then closes within
  `DOOR_PUSH_MAX_MS` = 2000): random appreciation. A door push with nobody
  close is ignored.
...
### lib/door.py (adapted from lib/bin_watch.py, which retires)
- `DoorLogic` (pure): fed `(beam_seen, now_ms)`. Beam blocked = door open.
  `is_open`, `just_pushed` (open→closed in under `DOOR_PUSH_MAX_MS`),
  `is_full` (open ≥ `FULL_AFTER_S`), `complain_due(now)` (once per
  `COMPLAIN_EVERY_S` while full).
- `Door` (wrapper): owns the `IRBeam` (unchanged), samples it in `update(now)`.
...
while True:
    now = ticks_ms()
    # 1. SENSE
    visitor.update(now); door.update(now); voice.update(now)
    # 2. DECIDE
...
Visitor: one zone value + a cooldown timestamp. Door: open/closed + when it
...
- Laser missing/dead at boot or mid-run → `Visitor` reads as `"away"`:
  greeting/passing/goodbye go quiet, the door behaviors still work
...
- `test_door.py`: push vs full classification, complaint cadence (port the
