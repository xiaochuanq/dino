...
| GP10 | HC-SR501 OUT | Push-pull 3.3 V output from the module - no pulls, no series resistor. VCC → VBUS (5 V), GND → GND. Jumper on H (repeat trigger), time-delay pot fully counter-clockwise (~3 s). Needs ~60 s warm-up after power-on; keep it away from the Pico W antenna and speaker wires. PIR unwired or floating low → greetings/passing noises/goodbyes silently never fire (the motion gate never opens); lid behaviors are unaffected. |
